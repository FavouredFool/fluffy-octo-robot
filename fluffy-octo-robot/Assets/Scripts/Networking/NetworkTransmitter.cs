...
public class NetworkTransmitter : NetworkBehaviour
{
    ...
    [ServerRpc]
    public void SendBytesToClientsServerRpc(int transmissionId, byte[] data)
    {
        Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
        SendBytesToClientsRoutineServerRpc(transmissionId, data);
    }

    [ServerRpc]
    public void SendBytesToClientsRoutineServerRpc(int transmissionId, byte[] data)
    {
        Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
        ...
        RpcPrepareToReceiveBytesClientRpc(transmissionId, data.Length);
        ...
        while (dataToTransmit.curDataIndex < dataToTransmit.data.Length - 1)
        {
            ...
            //send the chunk
            RpcReceiveBytesClientRpc(transmissionId, buffer);
    ...
    [ClientRpc]
    private void RpcPrepareToReceiveBytesClientRpc(int transmissionId, int expectedSize)
    {
    ...
    [ClientRpc]
    private void RpcReceiveBytesClientRpc(int transmissionId, byte[] recBuffer)
    {
